...
- Обновлены зависимости.
    - Прекращена поддержка python 3.10.

### [2.0.0b18] - 2025-07-09
- Переработана обработка формул контролей.
    - Реализован визитор, выполняющий обход дерева и формирование элементов и контекста до самих вычислений.
- Исправлено формирование контекста.
    - Теперь в него не попадают числа, являющиеся параметрами функций.
- Немного рефакторинга.

### [2.0.0b17] - 2025-07-02
- Обновление зависимостей.
- Переход с poetry на uv.

### [2.0.0b16] - 2025-07-01
- Исправлена проверка обязательных к заполнению значений.
- Добавлено условие пропуска проверки формата отчёта.
    - Пропуск происходит только если отчёт пустой и это позволено шаблоном.

### [2.0.0b15] - 2025-06-30
- Исправлена валидация атрибутов года и периодичности.
- Изменено условие пропуска проверки контроля со значениями за прошлый период.
    - Теперь пропуск срабатывает и на формуле условия.
- Ещё раз оптимизировано форматирование ошибок.

### [2.0.0b14] - 2025-06-10
- Реализован класс выполняющий роль хранения контекста для функции суммирования.
- Определён парсер для разбора отчётов и схем.
    - Сконфигурирован так, чтобы не делать лишнюю работу.

### [2.0.0b13] - 2025-06-03
- Добавлена проверка соответствия версии схемы и проверяемого по ней отчёта.

### [2.0.0b12] - 2025-05-30
- Доработан разбор формулы проверки периода.
    - Разбор формул вида `(&NP in(12))` приводил к ошибке.
- Исправлена проверка на равенство массивов в функции `nullif_`.

### [2.0.0b11] - 2025-05-13
- Исправлено форматирование ошибок.
    - Сделано уравнивание размерности операндов.
        - Позволяет полностью пройтись по всем парам операндов в случаях когда один из них скаляр.
    - Конечная маска больше не формируется из сложения результата проверки контроля и её маски.
        - Кроме того она не укрывает дельту, а участвует при итерации по операндам.
    - Удалено округление значений всех операндов и дельты.

### [2.0.0b10] - 2025-05-07
- Доработан метод развёртывания кодов координат и специфик.
    - Удалены флаги, определяющие особые условия форматирования и проверки токена.
        - Эти условия учтены в отдельных методах, вынесенных в потомков класса `Extendable`.
    - Для токенов специфик добавлена проверка значения по умолчанию и принадлежность к ключевым (grv).
        - Если оба условия не выполнены, проверка контроля прерывается.

### [2.0.0b9] - 2024-11-12
- Доработано форматирование ошибок прохождения контролей.
    - Исключение теперь является итератором, возвращающим объект со значениями каждой пары операндов не прошедших проверку.
- Добавлено исключение-прерывание, возбуждаемое при ошибке выполнения среза при развёртывании диапазона координат или спицифик.
- Изменено условие при создании элемента добавленное в 2.0.0b2 (0c034bc).
    - Пустой список данных заменяется на массив заполненный "nan", размерностью равной координатам из формулы.

### [2.0.0b8] - 2024-10-16
- Частичный откат изменений сделанных в 2.0.0b5 (1cf2496).
    - Удалено расширение размерности кодами колонок, которые якобы изменили свой тип по умолчанию.
    - Проверка контроля **не** прерывается если код не входит в размерность, он пропускается.
- Добавлена проверка финального списка координат.

### [2.0.0b7] - 2024-10-04
- Сделаны доработки для проверки формул с логической OR-функцией.
    - При наличии в формуле такого токена, проверка перестаёт быть "ленивой".
    - В объект дерева добавлен флаг процессинга.
        - Становится `True` перед применением к его ветвям трансформаций.
        - Служит флагом принудительной проверки при достижении корня.
- Исправлена проверка результата булевой операции при проверке правила.
- Метод трансформации, выполняющий булеву операцию, теперь возвращает сам результат, вместо маскировки операнда.

### [2.0.0b6] - 2024-10-02
- Исправлен разбор кодов координат и их проверка на вхождение в размерность.
    - Лидирующие нули теперь удаляются у всех, включая коды колонок.
    - Код форматируется перед проверкой.
- Исправлена проверка формулы периода.
    - В формулу теперь подставляется код периодичности.
    - Так как коды в формулах могут иметь лидирующие нули, все они теперь приводятся к числу.
- Поправлена проверка результата проверки формулы условия.
    - Теперь проверяются не значения массива, а инвертированная маска.

### [2.0.0b5] - 2024-09-27
- Реализована проверка на вхождение в размерность каждого кода координат при разборе формул контроля.
    - Проверка контроля прерывается если код не входит в список.
- Доработано определение и хранение кодов строк/колонок размерности.

### [2.0.0b4] - 2024-09-24
- Доработаны разбор и применение специфик при инициации элемента.
    - Добавлено получение/установка значения по умолчанию.
    - Добавлено дополнительное условие определения справочника со спецификами.
    - Набор специфик теперь может применятся не только к строке, но и к колонке.

### [2.0.0b3] - 2024-09-23
- Реализованы методы `__floor__` и `__bool__` для `nfloat`.
- Реализован класс `MaskedElement` наследующийся от `MaskedArray`.
    - Единственная задача, не терять координаты маскируемого элемента (если они есть).
- В очередной раз поправлена функция `sum_`.
    - Условие проверки пустого массива более не применимо из-за предыдущих изменений.
        - Любой массив размером в единицу или скаляр вернётся в исходном виде.
    - Контекстный элемент может быть замаскирован, что накладывает одно ограничение.
        - Нельзя определить его как `nfloat`, что приводит к сравнению наборов колонок и строк, которых у него быть не может.
- Исправлена опечатка в имени метода проверки строкового значения ячейки.

### [2.0.0b2] - 2024-09-19
- Доработан метод маскировки результата проверки операндов.
    - Маска результата проверки условия не всегда той же длины, что не позволяет её применить.
        - Причина в совершенно разных формулах условия и правила.
- Удалено прерывание проверки при отсутствии данных в элементе.
    - У второго элемента-операнда данные вполне могут быть поэтому проверку нельзя пропускать.
    - Пустой список данных заменяется скаляром "nan" при создании элемента.
        - Необходимо для предотвращения ошибок, связанных с размерностью.

### [2.0.0b1] - 2024-09-19
- Изменена логика проверки контроля.
    - Теперь достаточно даже одного элемента прошедшего условие для проверки правила.
    - Результат проверки условия участвует при проверке правила в роли маски.
        - "Укрытые" маской значения не учитываются при проверках.
- Реализована новая логика для операций проверки условий и правил.
    - Отдельные методы проверки результатов.
    - Для правил.
        - Укрытие маской результата проверки операндов.
        - Укрытие маской операнда в результате всех проверок.
    - Независимые исключения для прерывания проверки конроля.
        - Добавлено прерывание при отсутствии в отчёте раздела из формулы.
        - Добавлено прерывание при отсутствии данных для проверки в элементе.
        - Отлов ошибки проверки правила переехал в `ControlValidator`.
- Исправлена проверка наличия необходимых разделов отчёта в `FormatValidator`.
- Изменён порядок условий в функции `sum_`.
    - Проверка общих колонок идёт раньше проверки строк.
    - Вернул блок else.

### [2.0.0b0] - 2024-09-14
- Обновлен README.
- Обновлено описание ошибок.

### [2.0.0a18] - 2024-09-13
- Доработаны трансформации, связанные с выполнением математических и логических операций.
    - Округление операндов происходит только перед логической операцией.
    - Определены все условия учёта погрешности результата логической операции.
        - Только для правил.
        - Только если значение отклонения больше нуля.
        - Только для сравнений с учётом равенства/неравенства.
        - Только если дельта больше нуля, но меньше либо равна значению отклонения.
        - [Все проверенные комбинации](docs/fault.md).
- Доработан `nfloat`.
    - Реализованы методы `__trunc__` и `__round__`.
    - Исправлены методы `__neg__` и `__abs__`.
        - Теперь они возвращают новый объект, а не меняют самого себя.
- Функция округления значений массива переопределена для задействования метода `__round__`.
    - Иначе numpy использует округление через математические операции.

### [2.0.0a17] - 2024-09-11
- Доработан разбор специфик.
    - Специфики обрабатываются отдельно для каждой строки элемента.
- Поправлена функция `sum_`.
    - Если на входе массив нулевой длины, вернётся `nan`.
    - При суммировании по осям результирующий массив сохраняет размерность как у входящего.
    - Убран блок else.

### [2.0.0a16] - 2024-09-10
- Переработаны трансформации координат и специфик.
    - Все операции перенесены в соответсвующие классы.
    - Исправлена обработка координат и специфик со звёздочкой.
    - Исправлена развёртка диапазона координат и специфик.
    - Добавлено удаление лидирующих нолей для координат разделов и строк.
- Все специфики теперь приводятся к нижнему регистру.
- Доработана итерация по строкам отчёта.
    - Возвращен учёт специфик.

### [2.0.0a15] - 2024-09-07
- Исправлена обработка формул контролей с последовательностью вложенных функций.
    - В трансформере в методе обработки функции добавлен вызов `_сall_partial` для аргументов.
        - Функции может быть передан аргумент который предварительно следует суммировать.
    - В методе `_call_partial` и функции `sum_`, передача контекстного элемента стала необязательной.
        - В таком случае будет суммирован весь массив элемента.
    - Поправлена функция `isnull_`.
        - На входе может быть результат выполнения другой функции, являющийся скаляром.
- Исправлена функция `coalesce_`.
- Метод `nfloat.isnan` теперь возвращает объект типа `numpy.bool`.
...
### [2.0.0a13] - 2024-09-05
- Реализован float-like класс `nfloat`, определяющий тип данных в отчёте.
    - Использует особые условия при выполнении операций с "_nan_".
    - Допускает деление на ноль!
    - [Подробнее](docs/nfloat.md).
- Обновлены все объекты работающие с данными отчёта для поддержки `nfloat`.
    - Методы разбора формулы.
    - Массив значений `Element`.
    - Функции выполняющие действия над массивами.
- Возвращён strip для формул контролей.
- Возвращена "операция" в текст ошибки прохождения контроля.
    - Временно убрано "форматирование" элементов.
- Пара фиксов опечаток.
...
- Рефакторинг `FormatValidator`,`SpecInspector`,`ValueInspector`.
    - Микрооптимизации.
        - Обращений к элементам отчёта, справочникам и прочим параметрам валидации.
        - Конвертаций типов данных.
        - Условий проверок.

### [2.0.0a10] - 2024-08-21
- Исправлена проверка ключевого поля заголовка.

### [2.0.0a9] - 2024-08-20
- Правки валидаторов под обновленные схему и отчёт.
    - Рефакторинг `AttrValidator`, `TitleValidator`.

### [2.0.0a8] - 2024-08-19
- Доработка парсинга отчёта.
    - Оптимизация чтения xml-дерева.
    - Собственный `MultiDict` заменён на одноименный из библиотеки multidict.

### [2.0.0a7] - 2024-08-19
- Для хранения размерности раздела добавлен отдельный класс `SchemaDimension`.
    - Помимо кодов колонок теперь сохраняются и коды строк, доступных для ввода данных.

### [2.0.0a6] - 2024-08-17
- Доработка парсинга схемы.
    - Оптимизация чтения xml-дерева.
    - Правки чтения атрибутов определяющих формат ячеек отчёта.
        - Нода `default-cell` теперь читается для каждой кололки включая колонку-специфику.
        - Изменён формат записи идентификаторов специфик для удобства определения кода колонки.
        - Убрана попытка приведения кода колонки к числу.
    - Правки чтения справочников.
        - Удалён вспомогательный класс `NestedDefaultdict` и заменён на `SchemaCatalog`.
        - Изменён формат хранения данных из справочников.
        - Список идентификаторов терминов теперь формируется после чтения справочника и отсортирован.
    - Параметр skip_warns переименован в alerts.

### [2.0.0a5] - 2024-08-08
- Поправлена грамматика лексера.
    - Явно прописаны имена возможных функций.
    - Допускается, что все выражения могут быть обёрнуты в скобки.

### [2.0.0a4] - 2024-08-06
- Рефакторинг `FormulaInspector`.

### [2.0.0a3] - 2024-08-06
- Удалена подмена значения погрешности для условий и правил.
- Изменено значение погрешности по умолчанию с -1 на 0 (следуя спецификации).
- Изменено условие проверки погрешности.
    - Проверка будет провалена даже если дельта значений элементов **равна** значению погрешности.
- Исправлено вычисление дельты при проверке погрешности.

### [2.0.0a2] - 2024-08-05
- Исправлены оси по которым происходит суммирование в функции `sum_`.
- Исправлена подготовка операндов, когда оба завёрнуты в функцию.
- Исправлена проверка погрешности.
    - Проверка справедлива только для операций сравнения.
- Исправлено развёртывание диапазона координат.
- Добавлено возбуждение исключения `VisitError` если оно не содержит `ControlFault`.

### [2.0.0a1] - 2024-08-05
- Замена ply на lark.
- Для работы с матрицами использован numpy.
